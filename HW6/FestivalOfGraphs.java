...
class FestivalOfGraphs
{
	...
	public List<Integer> DepthFirstList(Integer v)
	{
		...
		vertices.add(v);
		dfs(v, vertices, visited);
		return vertices;
	}

	private final void dfs(Integer v, List<Integer> vertices, List<Integer> visited) {
		visited.add(v);
		for (Integer a : g.getAdjList(v).keySet()) {
			if (!visited.contains(a) && g.hasEdge(a, v)) {
				dfs(a, vertices, visited);
				vertices.add(a);
			}
		}
	}
	...
	public List<Integer> BreadthFirstList(Integer v) 
	{
		...
		while (!toExplore.isEmpty()) {
			Integer e = toExplore.pop();
			for (Integer a : g.getAdjList(e).keySet()) {
				if (!visited.contains(a) && g.hasEdge(a, e)) {
					toExplore.addLast(a);
					visited.add(a);
					vertices.add(a);
				}
	...
	private final void dfsTree(Integer v, Graph t, List<Integer> visited) {
		visited.add(v);
		for (Integer a : g.getAdjList(v).keySet()) {
			if (!visited.contains(a) && g.hasEdge(a, v)) {
				int weight = g.getEdgeWeight(a, v);
				t.addEdge(a, v, weight);
				dfsTree(a, t, visited);
			}
		}
	}
	...
	public Graph BreadthFirstSpanningTree(Integer v) 
	{
		...
		while (!toExplore.isEmpty()) {
			...
			for (Integer a : g.getAdjList(e).keySet()) {
				if (!visited.contains(a) && g.hasEdge(a, e)) {
					int weight = g.getEdgeWeight(a, e);
					visited.add(a);
					t.addEdge(a, e, weight);
					toExplore.addLast(a);
				}
			}
		}
