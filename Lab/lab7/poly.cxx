...
namespace main_savitch_5
{
    ...
    double polynomial::definite_integral(double a, double b) const
    {
		polynomial p_integral;
		unsigned int expo = 0;
		double coef;
		
		do
		{
			coef = coefficient(expo)/(expo + 1);
			p_integral.assign_coef(coef, expo+1);
			expo = next_term(expo);
		} while (expo != 0);
		
		return p_integral.eval(b) - p_integral.eval(a);
	}
    ...
    bool operator == (const polynomial& p1, const polynomial& p2)
    {
		...
		{
			unsigned int expo = 0;
			while(expo != p1.degree())
			{
				polynode x = p1.next_term(expo);
				polynode y = p2.next_term(expo);
				if((x.coef() != y.coef())||(x.exponent()!= y.exponent()))
				{	
					return false;
				}
				expo++;
