...
namespace Lab5
{
    ...
    unsigned int Aligner::match()
    {
		for (unsigned int q = 1; q < rows + 1; ++q)
		{
			...
			{
				unsigned int d = matches[q-1][r-1] + 1;
				unsigned int a = matches[q-1][r];
				unsigned int l = matches[q][r-1];
				...
				if (sequence1[q - 1] == sequence2[r - 1])
				{
					...
					// WRITE CODE TO FIND MAX OF d, a, OR l 
					// ASSIGN THIS MAX TO matches[q][r]					
				}
				// possibility 1: 2 letters do not match 
				else
				{
					if(a>l)
						matches[q][r] = a;
						...
						matches[q][r] = l;
					// WRITE CODE TO FIND MAX OF a OR l 
					// ASSIGN THIS MAX TO matches[q][r]
				}
			}
		}
